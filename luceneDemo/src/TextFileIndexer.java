...
public class TextFileIndexer {
    public static void main(String[] args) throws IOException {
        ...
        Document[] doc1List = new Document[10];
        /*
         ...
         */
        for (int i = 0; i < 10; i++) {
            doc1List[i] = new Document();
            doc1List[i].add(new Field("name", "Jianjun Lv " + (i + 1), TextField.TYPE_STORED));
            doc1List[i].add(new Field("sex", "male", TextField.TYPE_STORED));
            doc1List[i].add(new Field("address", "Microsoft Suzhou", TextField.TYPE_STORED));
            doc1List[i].add(new Field("dosometing", "I am learning lucene ", TextField.TYPE_STORED));
            writer.addDocument(doc1List[i]);
        }
...
//        Query query = new TermQuery(new Term("dosometing", "lucene"));
        Query query = new TermQuery(new Term("sex", "male"));
//         Query query = new TermQuery(new Term("address", "中国上海"));
//         Query query = new TermQuery(new Term("name", "Lv"));
        ...
        TopDocs rs = searcher.search(query, 5);
